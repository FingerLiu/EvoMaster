...
class MultiAgentPlayground(BasePlayground):
    ...
    def __init__(self, config_dir: Path = None, config_path: Path = None):
        ...
        Args:
            config_dir: 配置目录路径，默认为 configs/hyperfib_multi_agent/
            config_path: 配置文件完整路径（如果提供，会覆盖 config_dir）
        """
        if config_path is None and config_dir is None:
            # 默认配置目录
            config_dir = Path(__file__).parent.parent.parent.parent / "configs" / "agent" / "hyperfib_multi_agent"
    ...
    def setup(self) -> None:
        self.logger.info("Setting up hyperfib multi-agent playground...")

        self._setup_session()
        self._setup_agents()

        self.logger.info("Hyperfib multi-agent playground setup complete")
